...
class UserConfig {
   ...
   </ label="Enable emulator frame", help="Enable or disable emulator frame", options="No", order=21 /> enable_pointer="No"; 
   ...
   </ label="Enable text frame", help="Show text frame", options="Yes,No", order=25 /> enable_frame="No"; 
...
//adjust the values below for the static preview video snap
   ...
   local snapbg=null;
   snapbg = fe.add_image( "static.mp4", flx*0.31, fly*0.144, flw*0.295, flh*0.75);
...
surface_snap.set_pos(flx*0.31, fly*0.144, flw*0.295, flh*0.75);
...
{
local point = fe.add_image("pointers/[Emulator]", flx*0.36, fly*0.244, flw*0.485, flh*0.495 );
...
if ( my_config["enable_gboxart"] == "Yes" )
::OBJECTS <- {
 boxart = fe.add_artwork("boxart", flx*0.3, fly*0.78, flw*0.1, flh*0.17),
}
...
::OBJECTS <- {
 cartart = fe.add_artwork("cartart", flx*0.38, fly*0.86, flw*0.07, flh*0.08 ),
}
...
//Title text info
local textt = fe.add_text( "[Title]", flx*0.325, fly*0.945, flw*0.6, flh*0.025  );
...
//display filter info
local filter = fe.add_text( "[ListFilterName]: [ListEntry]-[ListSize]  [PlayedCount]", flx*0.120, fly*0.97, flw*0.3, flh*0.02 );
