...
class UserConfig {
   </ label="Select bg image or panscan", help="Select bg image or panscan art(flyer)", options="blue,panscan", order=1 /> enable_image="blue";   
   </ label="Select cab skin", help="Select a cab skin image", options="pal,ntsc", order=2 /> enable_cab="pal";
...
{
	snapbg = fe.add_image(
		"static.mp4", flx*0.044, fly*0.24, flw*0.31, flh*0.423);
...
surface.set_pos(flx*0.044, fly*0.24, flw*0.31, flh*0.423);
surface.skew_y = 0;
surface.skew_x = 0;
surface.pinch_y = 0;
surface.pinch_x = 0;
...
//cabinet image
if ( my_config["enable_cab"] == "pal" )
{
 local cab = fe.add_image( "robo.png", 0, 0, flw, flh );
}

if ( my_config["enable_cab"] == "ntsc" )
{
...
{
local boxart = fe.add_artwork("boxart", flx*0.37, fly*0.68 flw*0.21, flh*0.265 );
}
