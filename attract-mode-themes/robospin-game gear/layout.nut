...
class UserConfig {
   </ label="Select bg image or panscan", help="Select bg image or panscan art(flyer)", options="pal,us,panscan", order=1 /> enable_image="pal";   
   </ label="Select cab skin", help="Select a cab skin image", options="robo-pal,robo-us", order=2 /> enable_cab="robo-pal";
   ...
   </ label="Select pointer", help="Select animated pointer", options="us,pal,none", order=14 /> enable_pointer="pal"; 
   ...
   </ label="Enable box art", help="Enable box art", options="Yes,No", order=19 /> enable_gboxart="No";
}  
...
if ( my_config["enable_image"] == "pal") 
{
local bg = fe.add_image( "bkg.png", 0, 0, flw, flh );
bg.alpha=255;
}

if ( my_config["enable_image"] == "us")
{
...
{
	snapbg = fe.add_image(
		"static.mp4", flx*0.15, fly*0.255, flw*0.19 flh*0.28);
...
surface.set_pos(flx*0.15, fly*0.255, flw*0.19 flh*0.28);
surface.skew_y = 0;
surface.skew_x = 5;
surface.pinch_y = 0;
surface.pinch_x = -5;
...
//cabinet image
if ( my_config["enable_cab"] == "robo-pal" )
{
 local cab = fe.add_image( "robo.png", 0, 0, flw, flh );
}

if ( my_config["enable_cab"] == "robo-us" )
{
  local cab = fe.add_image( "robo2.png", 0, 0, flw, flh );
} 
...
if ( my_config["enable_pointer"] == "us") 
{
local point = fe.add_image("pointers/pointer.png", flx*0.895, fly*0.34, flw*0.20, flh*0.35);
...
if ( my_config["enable_pointer"] == "pal") 

 {
 local point = fe.add_image("pointers/pointer2.png", flx*0.91, fly*0.34, flw*0.1, flh*0.35);
 local alpha_cfg = {
    when = Transition.ToNewSelection,
    property = "alpha",
    start = 110,
...
{
local boxart = fe.add_artwork("boxart", flx*0.01, fly*0.60, flw*0.18, flh*0.35 );
}
