...
class UserConfig {
	...
	</ label="Scanline filter", help="", options="On,Off", order=6 /> scanline="Off"
...
local textemuW = flw - crw - bbm - floor( bbh * 2.875 )
local textemuH = floor( bbh * 0.15 )
local textemu = fe.add_text( "[Emulator]", flx + bbm + gameYearW/1.5, flh - bbm - textemuH, textemuW, textemuH );
textemu.set_rgb( 225, 255, 255 );
textemu.align = Align.Left;
textemu.style = Style.Regular
textemu.nomargin = true
textemu.charsize = floor(textemu.height * 1000/700)
textemu.font = flh <= 600 ? "BebasNeueRegular.otf": "BebasNeueBook.otf"
